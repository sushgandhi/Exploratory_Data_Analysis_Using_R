...
elect_data_cand_perc_prim <- filter(elect_data_cand_perc_prim,n >10000)
ggplot(elect_data_cand_perc_prim, aes(x="", y=freq, fill=cand_nm))+
geom_bar(width = 1, stat = "identity")+
  coord_polar("y",start=0)+
  labs(title = "Contribution distribution by candidates(Primaries)", y = "Count")+
...
ggplot(elect_data_cand_perc_gen, aes(x="", y=freq, fill=cand_nm))+
geom_bar(width = 1, stat = "identity")+
  coord_polar("y",start=0)+
  labs(title = "Contribution distribution by candidates(General)", y = "Count")+
  theme(plot.title = element_text(hjust = 0.5))+
       guides(fill = guide_legend(title = "Candidate"))
```
...
ggplot(aes(x = party,y = contbr_occupation), data = elec_campaign_occup_contrib)+
  geom_point(aes(size = cand_funds))+
  scale_size_continuous()+
  ggtitle("Contributions per Occupation and Party")+
  labs(y = 'Occupations', x = "Party")+
  theme(legend.position = "none")
```
...
```{r, echo=FALSE}
ggplot(aes(x = days_from_election,y = cumsum(contb_receipt_amt),color = cand_nm), data = subset(elec_data_exp,elec_data_exp$days_from_election %in% c(10,30,50,70,90,110) ))+
  geom_point()+
