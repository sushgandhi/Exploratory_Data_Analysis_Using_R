...
ggplot(aes(x = days_from_election), data = elec_data_exp) + 
  ...
  xlim(0,500)+
  labs(title = "Contribution with respect to days",x = "Days from Eleciton", y = "Count")+
...
ggplot(data = elect_data_cand_perc_prim,aes(x=cand_nm,y=freq))+
 ...
 theme(axis.text.x = element_text(angle = 90,  hjust = 0.5),legend.position="Centre")+
    labs(title = "Contribution distribution by candidates(Primaries)",y = "Percentage",x = "Candidate")
```
...
elect_data_cand_perc_gen <-  elec_data_exp %>%
  filter(election_tp == 'G2016' & (cand_id == 'P80001571' | cand_id == 'P00003392'))%>%
  group_by(cand_nm) %>%
  summarise (n = n()) %>%
  mutate(freq = (n / sum(n))*100)
...
ggplot(data = elect_data_cand_perc_gen,aes(x=cand_nm,y=freq))+
 ...
 theme(axis.text.x = element_text(hjust = 0.5))+
  theme(plot.title = element_text(hjust = 0.5))+
 labs(title = "Contribution distribution by candidates(General)",y = "Percentage",x = "Candidate")
```
...
```{r, echo=FALSE}
ggplot(aes(x = election_tp),data = group_by(subset(elec_data_exp,election_tp %in% c('G2016','P2016')),election_tp))+
...
ggplot(aes(x = cand_nm, y = avg_fund,fill = ifelse(party == 'Democrat','Red','Blue')), data = elec_campaign_avg_contrib)+
  ...
  labs(title = "Average Contribution per Candidate",x = "Candiate", y = "Average Contribution")+
  theme(plot.title = element_text(hjust = 0.5),legend.position="none")+
  theme(axis.text.x = element_text(angle = 90 ,hjust = 0.5),legend.position="none")
...
elec_campaign_occup_contrib <- elec_data_exp%>%
  filter(contbr_occupation != 'INFORMATION REQUESTED' & contbr_occupation != 'INFORMATION REQUESTED PER BEST EFFORTS')%>%
group_by(contbr_occupation,party)%>%
summarise(cand_funds = sum(contb_receipt_amt))

elec_campaign_occup_contrib <- filter(elec_campaign_occup_contrib,cand_funds > 100000)

ggplot(aes(x = contbr_occupation,y = cand_funds,fill = ifelse(party == 'Democrat','Red','Blue')), data = elec_campaign_occup_contrib)+
...
summarise(cand_funds = sum(contb_receipt_amt))
  ...
  ggplot(aes(x = days_from_election,y = cand_funds,color = cand_nm), data = elec_data_day_cum)+
    geom_line()+
...
ggplot(aes(x=gr,y=cand_funds,group=cand_nm,color = cand_nm), data = subset(elec_party_trend_cand,elec_party_trend_cand$cand_funds > 0))+
  geom_line()+
  labs(title = "Average contribution for Candidates over time",x = "Months before the Election", y = "Avg Contribution Amount",color = "Candidate")+
...
geom_bar( stat = "identity")+
  labs(title = "Contribution distribution by candidates", y = "Percentage Count")+
  theme(plot.title = element_text(hjust = 0.5))+
...
ggplot(aes(x=gr,y=cand_funds,group=cand_nm,color = cand_nm), data = subset(elec_party_trend_cand,elec_party_trend_cand$cand_funds > 0))+
  geom_line()+
  labs(title = "Average contribution for Candidates over time",x = "Months before the Election", y = "Avg Contribution Amount [USD]",color = "Candidate")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
